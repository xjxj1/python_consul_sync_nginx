# -*- coding: utf-8 -*-
# @Time    : 2020-05-12 21:59
# @Author  : wxl
# @Site    : 
# @File    : dynamic-service.py
# @Software: PyCharm
...
c = consul.Consul(host='172.19.2.62',port=8500)
...
NGINX_DYUPS_ADDR = "http://172.19.2.58:8081"
...
UPSTREAM_FILE = "/app/tengine/conf/conf.d/upstream.conf"
    ...
    for service_name in services_names:
	    ...
	    service_upstream[upstream_name] = []
            for service in service_nodes:
                ...
                service_check = [service_check for service_check in service['Checks']]
                if service_check:
                    service_status = service_check[0]['Status']
		    if service_status == 'passing':
                        logging.info("%s server %s status is passing, this server will updated into nginx upstream." % (upstream_name, service_addr))
                        service_upstream[upstream_name].append(service_addr)
                    else:
                        logging.warning(" %s server %s status is %s , this server will not be updated into nginx upstream" % (upstream_name, service_addr, service_status))
                else:
                    logging.error("%s instance %s does't have health check defined." % (service_name, service_addr))
...
def wx_callback():
    wx_url = 'https://qyapi.weixin.qq.com/cgi-bin/webhook/send?key=xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx'
    """艾特全部，并发送指定信息"""
    message = "生产环境 Can't connect to consul server, please check it and keep service alive"
